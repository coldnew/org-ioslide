...
=ox-ioslide-helper.el= is also available (depend on =makey.el=, install it if not), just =(require 'ox-ioslide-helper)=; which help you enter verbose properties/options/attributes into your slide file.
